        ...
        NavigationStack {
            ZStack {
                ...
                if store.items.isEmpty {
                    ContentUnavailableView("No Expenses Yet", systemImage: "tray", description: Text("Tap + to log your first one."))
                } else {
                    List {
                        ForEach(store.items) { item in
                            HStack {
                                VStack(alignment: .leading, spacing: 4) {
                                    ...
                                    Text("Amount ($): " + String(format: "%.1f", item.value2))
            ...
            .navigationTitle("Trip Budget Jar")
            .toolbar {
...
struct AddItemView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    TextField("Expense", text: $titleText)
                        .focused($focusedField, equals: .title)
                        .accessibilityIdentifier("addTitleField")
                    TextField("Amount ($)", text: $value2Text)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let item = Expense(title: titleText.isEmpty ? "Expense" : titleText, value2: Double(value2Text) ?? 0, notes: notesText)
                        if store.add(item) {
                            dismiss()
                        }
                    }

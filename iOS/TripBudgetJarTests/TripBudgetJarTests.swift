import XCTest
@testable import TripBudgetJar

@MainActor
final class TripBudgetJarTests: XCTestCase {
    var store: Store!
    ...
    func testAddItemIncreasesCount() {
        ...
        let added = store.add(Expense(title: "Test", value2: 0))
        XCTAssertTrue(added)
    ...
    func testCannotAddBeyondFreeLimitWhenNotPro() {
        ...
        for i in 0..<Store.freeLimit {
            _ = store.add(Expense(title: "Item \(i)", value2: 0))
        }
        let result = store.add(Expense(title: "Overflow", value2: 0))
    ...
    func testDeleteRemovesItem() {
        let item = Expense(title: "ToDelete", value2: 0)
        _ = store.add(item)
    ...
    func testUpdateModifiesItem() {
        var item = Expense(title: "Original", value2: 0)
        _ = store.add(item)
        item.title = "Updated"
        store.update(item)
    ...
    func testDeleteAtOffsets() {
        _ = store.add(Expense(title: "A", value2: 0))
        _ = store.add(Expense(title: "B", value2: 0))
    ...
    func testPersistenceRoundTrip() {
        _ = store.add(Expense(title: "Persisted", value2: 0))
        store.save()
        let reloaded = Store()
